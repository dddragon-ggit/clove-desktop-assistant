Create a single horizontal sprite strip for the Codex app digital pet `mudie` in the state `waiting`.
...
Repair attempt 1:
- The previous `waiting` strip failed QA: waiting used extraction method slots; regenerate the row or inspect slot slicing

Create a single horizontal sprite strip for the Codex app digital pet `mudie` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 无畏契约角色暮蝶风格的Q版桌宠，短发带蓝粉渐变，灰眼，表情灵动调皮，中性偏可爱，整体像小型数字宠物，不带复杂武器，保留一点蝴蝶和不朽气质。.
...
Repair attempt 1:
- The previous `waving` strip failed QA: waving frame 00 has 1100 non-transparent pixels close to the chroma key; waving frame 02 has 926 non-transparent pixels close to the chroma key; waving frame 03 has 1120 non-transparent pixels close to the chroma key

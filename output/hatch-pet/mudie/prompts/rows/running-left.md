Create a single horizontal sprite strip for the Codex app digital pet `mudie` in the state `running-left`.
...
Repair attempt 1:
- The previous `running-left` strip failed QA: running-left used extraction method slots; regenerate the row or inspect slot slicing

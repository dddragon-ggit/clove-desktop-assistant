Create a single horizontal sprite strip for the Codex app digital pet `mudie` in the state `failed`.
...
- This pet must remain a chibi anime-styled girl, not an animal, blob, mascot creature, seedling, rock, plush, or quadruped.
- Preserve the same short blue-violet to pink gradient hair, large gray eyes, human face, butterfly hair accessory placement, kimono-like outer layer, and tiny biped body from the canonical base.
- Never replace the hairstyle, species, clothing silhouette, or facial structure with a new interpretation.

Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 无畏契约角色暮蝶风格的Q版桌宠，短发带蓝粉渐变，灰眼，表情灵动调皮，中性偏可爱，整体像小型数字宠物，不带复杂武器，保留一点蝴蝶和不朽气质。.

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: 保持超简化Q版、粗描边、低细节、可爱但不要幼稚，桌面宠物感强，透明背景友好。.
...
Repair attempt 1:
- The previous `failed` strip failed QA: failed frame 01 has 886 non-transparent pixels close to the chroma key; failed frame 02 has 892 non-transparent pixels close to the chroma key; failed frame 03 has 1150 non-transparent pixels close to the chroma key; failed frame 04 has 1096 non-transparent pixels close to the chroma key; failed frame 05 has 1018 non-transparent pixels close to the chroma key; failed frame 06 has 1189 non-transparent pixels close to the chroma key; failed frame 07 has 849 non-transparent pixels close to the chroma key
